public class Main {
    public static void main(String[] args) {
        ...
        Exercise e1 = new Exercise("Push-ups","Place your hands on the floor. Raise up onto your toes so that all of your body weight is on your hands and your feet. Bend your elbows and lower your chest down toward the floor. Then, push off the floor and extend them so that you return to starting position.",30,"floor");
        ...
        Exercise allExercise[] ={e1,e2,e3,e4,e5,e6};

        for (int i = 0; i < allExercise.length; i++){
            System.out.println(allExercise[i].printExercise()+ "\n");
        }

        //print Exercises on the floor

        for (int i = 0; i <allExercise.length; i++){
            if (allExercise[i].getPosition() == "floor"){
                System.out.println(allExercise[i].printExercise()+ "\n");
            }
        }

        //print Exercise on the floor and > 1min

        for (int i = 0; i < allExercise.length; i++){
            if (allExercise[i].getPosition() == "floor" && allExercise[i].getDuration()>60){
                System.out.println(allExercise[i].printExercise()+"\n");
            }
        }
